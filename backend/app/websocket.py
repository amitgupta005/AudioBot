...
async def websocket_handler(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            # Process with Agent
            try:
                # 1. Get raw history from database
                raw_conversation = memory.get_conversation(conversation_id)
                
                # 2. Sanitize dicts into LangChain-friendly tuples
                formatted_conversation = []
                if raw_conversation:
                    for msg in raw_conversation:
                        if isinstance(msg, dict):
                            # Convert dict to a tuple: ("user", "hello")
                            role = msg.get("role", "user")
                            content = str(msg.get("content", ""))
                            formatted_conversation.append((role, content))
                        else:
                            # If it's already a LangChain object or tuple, keep it
                            formatted_conversation.append(msg)

                # 3. Pass the sanitized history to the state
                state = {
                    "user_input": str(user_text), # Enforce string type
