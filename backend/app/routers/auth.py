...
@router.post("/guest", status_code=status.HTTP_201_CREATED, response_model=TokenResponse)
async def create_guest(db: AsyncSession = Depends(get_db)):
    import uuid
    guest_id = str(uuid.uuid4())
    guest_email = f"guest_{guest_id}@guest.audiobot.local"
    user = User(
        id=guest_id,
        email=guest_email,
        password_hash=await hash_password(guest_id),
