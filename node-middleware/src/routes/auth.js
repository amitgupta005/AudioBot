...
router.post(
  ...
  [
    ...
    body('jobId').optional().isString().trim().notEmpty().withMessage('jobId must be a string'),
  ],
  async (req, res) => {
    ...
    try {
      ...
      const user = await User.create({ name, email, password, jobId: jobId || null, companyId });
